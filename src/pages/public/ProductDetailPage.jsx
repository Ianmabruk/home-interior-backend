...
export const ProductDetailPage = () => {
      ...
      <div className="flex min-h-[60vh] items-center justify-center bg-cream px-4">
        ...
        >
          <div className="mx-auto mb-6 flex h-16 w-16 items-center justify-center rounded-full bg-amber-100">
            <AlertCircle size={28} strokeWidth={1.5} className="text-amber-600" />
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
            <button
              onClick={retry}
              className="group inline-flex items-center gap-2 rounded-full bg-espresso px-6 py-3 text-[11px] font-semibold uppercase tracking-[0.08em] text-cream transition-all duration-300 hover:bg-amber-900 hover:shadow-[0_8px_30px_rgba(184,138,90,0.15)] hover:-translate-y-0.5"
            >
            ...
            <Link
              to="/shop"
              onClick={goBack}
              className="group inline-flex items-center gap-2 rounded-full border border-linen/60 bg-cream px-6 py-3 text-[11px] font-semibold uppercase tracking-[0.08em] text-charcoal transition-all duration-300 hover:border-gold hover:text-gold"
            >
  ...
  return (
    ...
    >
      {/* Breadcrumb */}
      <div className="border-b border-linen/40 bg-cream/60">
        <div className="container-wide px-4 py-3 md:px-12 lg:px-20">
          <nav className="flex items-center gap-2 text-2xs text-stone-500" aria-label="Breadcrumb">
            ...
            <ChevronRight size={12} strokeWidth={1.5} />
            <span className="text-charcoal truncate max-w-[200px] block">{product.name}</span>
          </nav>
        </div>
      </div>
        ...
        <div className="container-wide px-4 md:px-12 lg:px-20">
          <div className="mx-auto max-w-7xl grid gap-10 lg:gap-16 lg:grid-cols-2">
            ...
            <motion.div variants={fadeUp} custom={0} className="space-y-4">
              <div className="relative aspect-[4/5] overflow-hidden rounded-[32px] bg-linen shadow-soft">
                ...
                {salePercent && (
                  <span className="absolute left-4 top-4 rounded-full bg-gold px-4 py-1.5 text-xs font-semibold uppercase tracking-widest text-cream shadow-lg">
                    -{salePercent}% Off
                  </span>
                )}
                {!inStock && (
                  <div className="absolute inset-0 flex items-center justify-center bg-charcoal/40 backdrop-blur-sm">
                    <span className="rounded-full bg-cream px-6 py-2 text-sm font-semibold uppercase tracking-widest text-charcoal">Sold Out</span>
                  </div>
                )}
              </div>
            ...
            <motion.div variants={fadeUp} custom={1} className="flex flex-col">
              <p className="text-2xs font-semibold uppercase tracking-widest text-gold">{product.category}</p>
              <h1 className="mt-3 font-display text-4xl font-medium leading-tight text-charcoal md:text-5xl">
                {product.name}
              </h1>
              ...
              <p className="mt-1 text-2xs font-semibold uppercase tracking-widest text-gold">
              ...
              {activeVariant?.sku && (
                <p className="mt-2 text-xs font-medium uppercase tracking-widest text-stone-450">SKU: {activeVariant.sku}</p>
              )}

              {/* Description */}
              {product.description && (
                <div className="mt-6">
                  <h3 className="text-xs font-semibold uppercase tracking-widest text-stone-500 mb-3">Description</h3>
                  <p className="text-sm leading-[1.8] text-stone-600">{product.description}</p>
                </div>
              )}
              ...
              {SPECIFICATION_FIELDS.some(field => product?.[field.key]) && (
                <div className="mt-8">
                  <h3 className="text-xs font-semibold uppercase tracking-widest text-stone-500 mb-4">Specifications</h3>
                      ...
                      .map((field, idx) => {
                        ...
                        return (
                          <motion.div
                            ...
                            className="flex items-start gap-3 rounded-xl bg-white p-4 border border-linen/60 shadow-sm"
                          >
                            <div className="flex-shrink-0 mt-0.5 flex h-10 w-10 items-center justify-center rounded-lg bg-gold/10 text-gold">
                              <Icon size={18} strokeWidth={1.5} />
                            </div>
                            <div>
                              <p className="text-2xs font-semibold uppercase tracking-widest text-gold">{field.label}</p>
                              <p className="text-sm text-charcoal mt-0.5">{displayValue}</p>
                            </div>
                          </motion.div>
                        )
                      })}
              ...
              {product.colorVariants?.length > 0 && (
                <div className="mt-8">
                  <p className="text-2xs font-semibold uppercase tracking-widest text-stone-500 mb-3">
                    Color: <span className="text-charcoal font-medium">{activeColor}</span>
                  </p>
                      ...
                      <button
                        ...
                        className={`relative h-12 w-12 min-h-[48px] min-w-[48px] overflow-hidden rounded-full border-2 transition-all duration-300 ${
                          activeColor === variant.colorName
                            ...
                            : 'border-linen/60 hover:border-gold/60'
                        }`}
                      >
                        {variant.imageUrl ? (
                        ...
                        {activeColor === variant.colorName && (
                          <span className="absolute inset-0 flex items-center justify-center rounded-full">
                            <Check size={14} strokeWidth={2.5} className="text-cream drop-shadow-md" />
                          </span>
                        )}
                      </button>
                    ))}
              ...
              <div className="mt-8 flex flex-wrap gap-3">
                ...
                <button
                  ...
                  className={`btn-outline flex items-center gap-2 ${isWishlisted ? 'border-gold text-gold' : ''}`}
                >
              ...
              {/* Features */}
              <div className="mt-8 flex flex-wrap gap-6 border-t border-linen/40 pt-6">
                <div className="flex items-center gap-2 text-sm text-stone-550">
                  <Truck size={18} strokeWidth={1.5} className="text-gold" />
                  <span>Free Delivery</span>
                </div>
                <div className="flex items-center gap-2 text-sm text-stone-550">
                  <Shield size={18} strokeWidth={1.5} className="text-gold" />
        ...
        <section className="section-pad bg-linen">
          <div className="container-wide px-4 md:px-12 lg:px-20">
            <motion.div
              ...
              className="mb-10"
            >
              <p className="text-2xs font-semibold uppercase tracking-widest text-gold mb-3">You May Also Like</p>
              <h2 className="font-display text-4xl font-medium text-charcoal md:text-5xl">Related Products</h2>
            </motion.div>
              ...
              {relatedProducts.map((p, i) => (
                ...
                >
                  <Link to={`/shop/${p._id}`} className="group block">
                    <div className="overflow-hidden rounded-2xl bg-cream shadow-soft group-hover:shadow-lift transition-all duration-500">
                      ...
                      <div className="p-4">
                        <p className="text-2xs font-medium uppercase tracking-widest text-gold">{p.category}</p>
                        <h3 className="font-display text-lg font-medium text-charcoal mt-1 group-hover:text-gold transition-colors">
                          {p.name}
                        </h3>
                      </div>
                    </div>
